...
| **Live bank paging** *(experimental)* | In the SELECT BANK screen, `[PAGE]` cycles through **sibling projects** (`<name>`, `<name>_2`, `_3`, `_4`) and, on confirmation, loads that project's 16 banks into RAM **without stopping the sequencer/audio** — so a live set can reach far more than 16 banks. Sibling projects must share the same sample pool. The core (background CF load, no audio stop) is hardware-validated; the full UX is a release candidate pending final on-device testing. See [`DESIGN_BANKPAGE.md`](DESIGN_BANKPAGE.md). |
