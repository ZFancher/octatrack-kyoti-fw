# Inherited octamax mod-design notes

> These sections were reverse-engineered and written during the octamax lineage
> (Maxolydian's behaviour mods, plus a shelved live-bank-paging experiment).
> **They are kept for reference only and are not part of OT Kyoti FW** — no OT
> Kyoti build carries any of these mods. The patch sources are in
> `tools/attic/`; the credit and lineage are in `CREDITS.md`. The consolidated,
> address-keyed map that supersedes the raw findings here is
> `reference/kb/memory-map.md`.

Relocated from `NOTES.md` during the 2026-09 repo reorganization; the old
`DESIGN_BANKPAGE.md` (live-bank-paging design) and `HANDOFF.md` were folded in
here / dropped. The patch sources these sections refer to (`patch.s`,
`patch_enc.s`, `patch_gui.s`, `patch_led.s`, `patch_notimer.s`, `patch_scene*.s`,
`patch_arp.s`, `build.py`, `build_arp.py`) are now under **`tools/attic/`**; the
shelved bank-paging experiment tooling, `emu_image.py`, and the standalone
scene/LED/editor emulators were removed (recoverable from git history or
`upstream/main`). References below to `patch_gui2.s` are to a file that was only
ever planned, not committed.
