...
| [sambanks/octabam](https://github.com/sambanks/octabam) | DSP + ColdFire | Adds DSP56300 effects to the MKII. The DSP side is out of scope here. **2026-09 "RTOS fork"** (`RTOS_FORK.md`): `emu_rtos.py` full-firmware emulator, kernel/scheduler decode, the per-step **TRAC mask map** (recorder trigs HW-confirmed), `ot_project.py` `pattern-trig`/`pattern-diff`, `PARAM_PAGES.md` full descriptor-table decode, `FAILURE_MODES.md`, `midi_flash.py`. **RTOS 10.13–10.16 (`47f6cc5`, 2026-09-07):** the **EMAC-patched Unicorn** (`build_unicorn.sh` — stock's fractional `macl` is half of hardware's), machine-type values `0=STATIC/1=FLEX/4=PICKUP`, the 5-byte per-track slot record, `0x80004f1c` recorder state record. → [`kb/memory-map.md`](kb/memory-map.md), [`kb/file-format.md`](kb/file-format.md), [`kb/techniques.md`](kb/techniques.md), [`kb/dsp56300.md`](kb/dsp56300.md) |
