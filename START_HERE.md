...
| **RELOAD FROM PROJECT** — per-pattern reload from the CF card, no transport stop (`WHOLE PATTERN` / `ALL PARTS` / `SEQ DATA`) | Session 42: **SEQ DATA built + emu-validated end to end.** `build_reload.py` → `OCTATRACK_OS1.40C_RELOAD.{syx,bin}` (571 B vs stock). `[PTN]`+`[NO]` while playing → async storage-task worker parses the one pattern from `bankNN.strd` via the firmware's own `FUN_4008cebc` (no scratch bank), memcpy → live slab, `0x46c8028a` seamless re-home. `emu_reload.py --combo` + `--patched` ALL GOOD (parse lands byte-exact vs disk, bystander pattern untouched, no fault, transport keeps running). Flash-ready ≈ DIRECT JUMP; HW procedure in `FLASHING.md` §4.7. ALL PARTS (`FUN_4004aab4(0..3)`, pure RAM) + WHOLE + 3-way picker = phase 2. NOT flashed. | `NOTES.md` "Session 42" |
