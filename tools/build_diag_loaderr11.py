#!/usr/bin/env python3
"""build_diag_loaderr11.py -- MEASURING build v10 (fixes nothing). WIDE instrumentation: every open
...
PROBE 0x40ab65e0 = project.256 offset 0x21000 (1008 B, fits inside ONE high slot's record):
    ...
    +0x004 cntB  +0x00c cntL  +0x018 cntLall  +0x01c cntP  +0x020 cntPall
    +0x024 cntAED  +0x02c cntAEDall
    +0x040 B[8]  [result][callerPC][a2]                     12 B
    +0x0a0 L[8]  [slot][result][pathptr][str16]             28 B
    +0x180 P[8]  [slot][type][handle]                       12 B  -- parser invoked for a high slot?
    +0x1e0 AED[8] [SETTINGS ptr][slice count][slice[0] word]  12 B  -- what the screen reads

DELETE <project>/project.256 before the run. The probe block overlaps ONE high slot -- printed below.

    python3 tools/build_diag_loaderr11.py    # -> out/mainos_diag_loaderr11.bin
"""
...
BASE_VA = 0x40000400
OUT = pathlib.Path("out/mainos_diag_loaderr11.bin")
...
p_probe:
    ...
    movea.l #0x{MAGIC:x},%a1
    move.l  %a1,(%a0)
    addq.l  #1,32(%a0)                  | cntPall -- every invocation, any slot
...
| ==== AED: 0x40070cd2 -- the AED SLICE tab reading the slice COUNT it will draw.
|      a3 comes from the migrated getter 0x4006da78 two instructions earlier, so a3 NAMES THE SLOT:
|        SET-A -> (a3-0x100d5b30)/0x448        SET-B -> 128 + (a3-SET_B)/0x448
|      UNFILTERED (ring 8 + total at +0x2c) so a LOW slot that draws slices and a HIGH slot that does
|      not can be compared side by side in one run. This settles whether +1092 is even the field the
|      screen uses: if a low slot shows a non-zero count here and the high slot shows 0, it is.
aed_probe:
    lea     -24(%sp),%sp
    movem.l %d0-%d3/%a0-%a1,(%sp)
    lea     0x{PROBE:x},%a0
    movea.l #0x{MAGIC:x},%a1
    move.l  %a1,(%a0)
    addq.l  #1,44(%a0)                  | total AED reads (+0x2c)
    moveq   #8,%d0
    moveq   #36,%d1
    move.l  #0x1e0,%d2
    moveq   #12,%d3
    ...
    tst.l   %d0
    beq.b   1f
    move.l  %a3,(%a1)                   | the SETTINGS record -> names the slot
    move.l  1092(%a3),%d0
    move.l  %d0,4(%a1)                  | the slice count the screen will use
    move.l  312(%a3),%d0
    move.l  %d0,8(%a1)                  | first slice word, as corroboration
1:  movem.l (%sp),%d0-%d3/%a0-%a1
    lea     24(%sp),%sp
    move.l  1092(%a3),%d3
    lea     0x460e5fe4,%a1
...
# Probe-block layout, declared ONCE here so the decoder cannot drift from the build.
# tools/read_probe.py imports this. Offsets are from PROBE (= project.256 offset 0x21000).
#   counter: byte offset of the ring's count      entry:   bytes per record
#   fields:  (name, offset-within-record, kind)   kind: "u32" | "s32" | "hex" | "ptr" | "str"
LAYOUT = {
    ...
    "L":   {"counter": 0x0c, "array": 0x0a0, "entry": 28, "cap": 8, "total": 0x18,
            "fields": [("slot", 0, "u32"), ("result", 4, "s32"), ("path", 8, "ptr"),
                       ("pathstr", 12, "str")]},
    "P":   {"counter": 0x1c, "array": 0x180, "entry": 12, "cap": 8, "total": 0x20,
            "fields": [("slot", 0, "u32"), ("type", 4, "u32"), ("handle", 8, "hex")]},
    "AED": {"counter": 0x24, "array": 0x1e0, "entry": 12, "cap": 8, "total": 0x2c,
            "fields": [("settings", 0, "ptr"), ("slicecount", 4, "u32"), ("slice0", 8, "hex")]},
}

HOOKS = [
    (0x40022b50, "4aaf00046c18",             "b_probe"),
    (0x40089940, "4fefffec48d70c1c",         "p_probe"),
    (0x40070cd2, "262b044443f9460e5fe4",     "aed_probe"),
...
def main():
    ...
    hi_idx = (poff + 0x300 - 1) // 0x448
    slots = list(range(129 + bidx, 129 + hi_idx + 1))
    print(f"  probe block spans 0x{poff:05x}..0x{poff + 0x300:05x} = SET-B[{bidx}..{hi_idx}]  ->  "
