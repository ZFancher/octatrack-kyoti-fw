#!/usr/bin/env python3
"""
blockmove base + a CHANGE-confirm hook that dumps the relocated STATIC settings block from
DDR to /BLK.BIN, to see whether the (failed) project load populated it. See patch_blockdump.s.

    python3 tools/build.py            # -> out/mainos.bin
    python3 tools/build_blockdump.py  # -> out/mainos_blockdump.bin
"""
...
OUT = pathlib.Path("out/mainos_blockdump.bin")
...
BD_CAVE = 0x400d7300
BD_DETOURS = [(0x40063e28, "dump_hook", "4aaf00046618")]
...
def main():
    ...
    print("boot-zero:"); inject(img, "tools/patch_bootzero.s", BZ_CAVE, [(BZ_DETOUR, "bootzero_stub", BZ_EXPECT)])
    print("block-dump:"); inject(img, "tools/patch_blockdump.s", BD_CAVE, BD_DETOURS)
