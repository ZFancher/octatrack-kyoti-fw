...
reserved window.
  ...
  neighbour to preserve.
    ...
    python3 tools/build_phase1.py # -> out/mainos_phase2a.bin
"""
...
OUT = pathlib.Path("out/mainos_phase2a.bin")

OLD_POOL, NEW_POOL = 0x40a955e0, 0x40b555e0     # +128 pages (768 KB reserved) for the 256 layout
...
def main():
    ...
    # Phase 2a: relocate the state table 0x46c90a78 (128 entries) to a 256-capable DDR home in
    # the reserved region (256*44 = 0x2c00). Blanket replace all 36 refs. Keep 128 count for now
    # (regression test). Stock never boot-zeroes it (DDR, rebuilt on load), so no boot-zero change.
    STATE_OLD, STATE_NEW = 0x46c90a78, 0x40b00000
    ns = img.count(STATE_OLD.to_bytes(4, "big"))
    if ns != 36:
        sys.exit(f"state-table refs {ns} != 36 -- aborting")
    img = bytearray(img.replace(STATE_OLD.to_bytes(4, "big"), STATE_NEW.to_bytes(4, "big")))
    print(f"Phase 2a: state table 0x{STATE_OLD:08x} -> 0x{STATE_NEW:08x} ({ns} refs, 128-count kept)")
    ...
    # boot-zero hook
    r = subprocess.run(["m68k-elf-as", "-mcpu=5407", "-o", "out/bootzero.o", "tools/patch_bootzero2.s"])
    if r.returncode:
