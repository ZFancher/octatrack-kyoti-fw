...
# registra el valor de D4 (track) en cada branch hacia atrás grande (loop externo)
prev=[0]; edges=Counter(); d4hist=[]
def hook(uc,address,size,user):
    if address<prev[0] and prev[0]-address>0x60:   # salto atrás grande = loop externo
        d4=uc.reg_read(UC_M68K_REG_D4)
        edges[(prev[0],address)]+=1
        d4hist.append((prev[0],d4))
    prev[0]=address
uc.hook_add(UC_HOOK_CODE,hook,begin=0x40009094,end=0x4000966a)
sp=0x41010000; RET=0x401f0000
for a in (1,0): sp-=4; uc.mem_write(sp,struct.pack(">I",a))
sp-=4; uc.mem_write(sp,struct.pack(">I",RET)); uc.reg_write(UC_M68K_REG_A7,sp)
try: uc.emu_start(0x40009094,RET,count=300000)
except UcError: pass
print("branches-atrás grandes (candidatos a loop externo de tracks):")
for (frm,to),c in edges.most_common(6):
    d4s=[d for f,d in d4hist if f==frm]
    print(f"  0x{frm:08x} -> 0x{to:08x}  (x{c})  valores D4 al saltar: {d4s[:9]}")
