...
reserved window.
  ...
  neighbour to preserve.
    ...
    python3 tools/build_blockmove2.py # -> out/mainos_blockmove2.bin
"""
...
OUT = pathlib.Path("out/mainos_blockmove2.bin")
...
def main():
    ...
    for i in range(2, N - 3):
        ...
        if not op:
            continue
        # RELOCATE EVERYTHING at 0x100f7f30 too (incl. the 2 pea) -> move the block-end
        # global (project-name buffer) into DDR so block-relative readers see the writes
        img[i:i + 4] = (v + BLK_DELTA).to_bytes(4, "big")
        reloc[v] += 1
