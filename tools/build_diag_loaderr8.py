#!/usr/bin/env python3
"""build_diag_loaderr8.py -- MEASURING build v7 (fixes nothing). WIDE instrumentation: every open
...
PROBE 0x40ab65e0 = project.256 offset 0x21000 (1008 B, fits inside ONE high slot's record):
    ...
    +0x080 X[12] [callerPC][strptr][str16]                  24 B
    +0x1a0 L[12] [slot][result][pathptr][str16]             28 B
    +0x2f0 C[8]  [type][slot][state8][state20]              16 B
    +0x370 A[8]  [slot][state8][type]                       12 B

DELETE <project>/project.256 before the run. The probe block overlaps ONE high slot -- printed below.

    python3 tools/build_diag_loaderr8.py    # -> out/mainos_diag_loaderr8.bin
"""
...
BASE_VA = 0x40000400
OUT = pathlib.Path("out/mainos_diag_loaderr8.bin")
...
| ==== X: 0x40013db0 = strlen. Replaces the four E probes, which were UNHOOKABLE: each -2 site is a
|      2-byte `moveq` whose NEXT address is itself a branch target (four `bra.b` land on 0x4008498e),
|      so a 6-byte jmp there lands a branch mid-instruction -> VEC:04. That is what P39 threw.
|      Hooking strlen instead covers ALL SIX guards in the loader function at once and needs no
|      register saving on the fast path: the replaced code writes both d0 and a0, so both are dead.
|      At entry sp@(0) = caller PC, sp@(4) = the string pointer.
x_probe:
    move.l  (%sp),%d0                   | caller PC (d0 is dead: replaced code does `clrl d0`)
    cmpi.l  #0x40084800,%d0
    blo.b   xout
    cmpi.l  #0x40084c00,%d0
    bcc.b   xout
    lea     -20(%sp),%sp
    movem.l %d1-%d3/%a0-%a1,(%sp)
    move.l  %d0,-(%sp)                  | keep the caller PC across rec_alloc
    moveq   #12,%d0
    moveq   #8,%d1
    move.l  #0x80,%d2
    moveq   #24,%d3
    ...
    tst.l   %d1
    beq.b   1f
    move.l  %d0,(%a1)                   | caller PC -> which guard asked
    move.l  24(%sp),%d0                 | the string pointer -> names the table and slot
    move.l  %d0,4(%a1)
    tst.l   %d0
    beq.b   1f
    movea.l %d0,%a0
...
HOOKS = [
    ...
    (0x40013db0, "206f00044280",             "x_probe"),
...
def main():
    ...
    hi_idx = (poff + 0x3d0 - 1) // 0x448
    slots = list(range(129 + bidx, 129 + hi_idx + 1))
    print(f"  probe block spans 0x{poff:05x}..0x{poff + 0x3d0:05x} = SET-B[{bidx}..{hi_idx}]  ->  "
