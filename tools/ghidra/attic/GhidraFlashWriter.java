...
public class GhidraFlashWriter extends GhidraScript {
    DecompInterface dec;
    ...
    public void run() throws Exception {
        ...
        Address a = sp.getAddress(0x4006370cL);
        Function root = fm.getFunctionContaining(a);
        if (root == null) { disassemble(a); root = createFunction(a, "os_flash_writer"); }
        if (root == null) { println("[!] no hay funcion en 0x40063660"); return; }
        try { root.setName("os_flash_writer", ghidra.program.model.symbol.SourceType.USER_DEFINED); } catch (Exception e) {}
