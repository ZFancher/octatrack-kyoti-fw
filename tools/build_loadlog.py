#!/usr/bin/env python3
"""
2afix base (pool move + settings->DDR, END pointers kept) + LOAD-PHASE breadcrumbs.
Writes 'A' at the reload-orchestrator entry and 'B' at the re-sync entry to /LP.TXT, so a
hung load leaves the last phase reached on the CF. See tools/patch_loadlog.s.

    python3 tools/build.py          # -> out/mainos.bin (R11)
    python3 tools/build_loadlog.py  # -> out/mainos_loadlog.bin
"""
...
OUT = pathlib.Path("out/mainos_loadlog.bin")
...
CAVE_AT = 0x400d7280
DETOURS = [(0x4009083c, "reload_stub", "4fefffd448d77cfc"),
           (0x400238a4, "resync_stub", "2f0a4eb94009b220")]
...
def main():
    ...
    # breadcrumbs: assemble, inject, detour
    r = subprocess.run(["m68k-elf-as", "-mcpu=5407", "-o", "out/loadlog.o", "tools/patch_loadlog.s"])
    if r.returncode:
        sys.exit("assembler failed")
    subprocess.run(["m68k-elf-ld", f"-Ttext=0x{CAVE_AT:x}", "-o", "out/loadlog.elf", "out/loadlog.o"],
                   capture_output=True)
    subprocess.run(["m68k-elf-objcopy", "-O", "binary", "out/loadlog.elf", "out/loadlog.bin"], check=True)
    nm = subprocess.run(["m68k-elf-nm", "out/loadlog.elf"], capture_output=True, text=True).stdout
    sym = {p[2]: int(p[0], 16) for p in (l.split() for l in nm.splitlines()) if len(p) == 3}
    blob = pathlib.Path("out/loadlog.bin").read_bytes()
    end = CAVE_AT + len(blob)
    print(f"breadcrumb cave {len(blob)} B @ 0x{CAVE_AT:08x}..0x{end-1:08x} (cave ends 0x400d7c3b)")
    if end > 0x400d7c3c:
    ...
    for site, s, exp in DETOURS:
        o = off(site)
        if bytes(img[o:o + 8]).hex() != exp:
            sys.exit(f"detour 0x{site:08x}: {bytes(img[o:o+8]).hex()} want {exp}")
