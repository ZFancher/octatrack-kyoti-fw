#!/usr/bin/env python3
"""build_diag_loaderr3.py -- MEASURING build v2 (fixes nothing). Replaces build_diag_loaderr.py, whose
...
populated slot). Do not use slot 247 -- the probe overlaps its record.
    ...
    python3 tools/build_diag_loaderr2.py    # -> out/mainos_diag_loaderr3.bin
"""
...
BASE_VA = 0x40000400
OUT = pathlib.Path("out/mainos_diag_loaderr3.bin")
CODE, PROBE = 0x400d6a00, 0x40ab65e0
MAGIC = 0x10ade111
...
# room for one: the extra NOP clobbered the first half of the next instruction (0x4009937c `moveal
# sp@(44),a4`), leaving `002c` to execute at 0x4009937e -> VEC:04 illegal instruction, on screen.
HOOKS = [
    (0x40022b50, "4aaf00046c18", "b_probe"),        # the live emitter (A never fired on HW)
    (0x40099374, "4fefffd848d73cfc", "sl_probe"),   # hardware-proven in P33D
]
...
def main():
    ...
    for va, exp, name in HOOKS:
        ...
        print(f"  hook 0x{va:08x} -> {name} @0x{sym[name]:08x}  (hole {hole} B, {(hole - 6) // 2} nop)")
