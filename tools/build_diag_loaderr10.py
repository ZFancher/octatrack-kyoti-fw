#!/usr/bin/env python3
"""build_diag_loaderr10.py -- MEASURING build v9 (fixes nothing). WIDE instrumentation: every open
...
PROBE 0x40ab65e0 = project.256 offset 0x21000 (1008 B, fits inside ONE high slot's record):
    ...
    +0x024 cntST  +0x028 cntO  +0x02c cntSTall
    ...
    +0x1e0 ST[8] [jobrec][slot][type]                       12 B  -- does the .ot state run at all?
    +0x240 O[8]  [openresult][jobrec][filename16]           24 B  -- did the .ot file open?

DELETE <project>/project.256 before the run. The probe block overlaps ONE high slot -- printed below.

    python3 tools/build_diag_loaderr10.py    # -> out/mainos_diag_loaderr10.bin
"""
...
BASE_VA = 0x40000400
OUT = pathlib.Path("out/mainos_diag_loaderr10.bin")
...
| ==== ST: 0x40084e7a -- entry of the state that opens the .ot and calls the parser. UNFILTERED
|      (ring 8) plus a total counter, because the question is whether this state runs AT ALL for the
|      high slot. a2 is the job record: a2@(2) = slot, a2@(6) = type (the same two the parser gets).
st_probe:
    lea     -24(%sp),%sp
    movem.l %d0-%d3/%a0-%a1,(%sp)
    lea     0x{PROBE:x},%a0
    movea.l #0x{MAGIC:x},%a1
    move.l  %a1,(%a0)
    addq.l  #1,44(%a0)                  | cntSTall (+0x2c) -- MUST differ from the ring's
    |                                     own counter at +0x24, or rec_alloc sees an already
    |                                     bumped count and skips entry 0.
    moveq   #8,%d0
    moveq   #36,%d1
    move.l  #0x1e0,%d2
    moveq   #12,%d3
...
1:  movem.l (%sp),%d0-%d3/%a0-%a1
    lea     24(%sp),%sp
    move.l  270(%a2),-(%sp)
    pea     0x40084e82                  | the original return address of `jsr %pc@(0x40084094)`
    jmp     0x40084094

| ==== O: 0x40084ea8 -- the .ot OPEN result. d0 < 0 here SKIPS the parser entirely (blt at
|      0x40084eae). Records [result][a2][16 bytes of the filename at a2@(10)].
o_probe:
    lea     -24(%sp),%sp
    movem.l %d0-%d3/%a0-%a1,(%sp)
    moveq   #8,%d0
    ...
    tst.l   %d0
    beq.b   1f
    move.l  (%sp),%d0                   | saved d0 = the open result
    move.l  %d0,(%a1)
    move.l  %a2,4(%a1)
    movea.l 10(%a2),%a0                 | the filename pointer
    move.l  %a0,%d0
    tst.l   %d0
    beq.b   1f
    lea     8(%a1),%a1
...
HOOKS = [
    ...
    (0x40084e7a, "2f2a010e4ebaf214",         "st_probe"),
    (0x40084ea8, "4fef00184a80",             "o_probe"),
